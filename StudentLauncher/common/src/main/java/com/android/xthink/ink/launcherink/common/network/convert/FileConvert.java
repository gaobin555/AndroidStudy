package com.android.xthink.ink.launcherink.common.network.convert;

import android.os.Environment;
import android.text.TextUtils;
import android.util.Log;

import com.android.xthink.ink.launcherink.common.network.InkHttpManager;
import com.android.xthink.ink.launcherink.common.network.InkHttpUtils;
import com.android.xthink.ink.launcherink.common.network.callback.AbsCallback;
...
public class FileConvert implements Converter<File> {
    ...
    public File convertSuccess(Response value) throws Exception {
        ...
        if (TextUtils.isEmpty(destFileName)) destFileName = InkHttpUtils.getNetFileName(value, value.request().url().toString());
        ...
        try {
            ...
            while ((len = is.read(buf)) != -1) {
                ...
                if (callback != null) {
                    ...
                    //每200毫秒刷新一次数据
                    if (curTime - lastRefreshUiTime >= InkHttpManager.REFRESH_TIME || finalSum == total) {
                        //计算下载速度
                        long diffTime = (curTime - lastRefreshUiTime) / 1000;
                        if (diffTime == 0) diffTime += 1;
                        long diffBytes = finalSum - lastWriteBytes;
                        final long networkSpeed = diffBytes / diffTime;
                        InkHttpManager.getInstance().getDelivery().post(new Runnable() {
                            @Override
                            public void run() {
