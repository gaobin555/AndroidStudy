...
public final class StringUtils {
    ...
    public static String capitalizeFirstLetter(String str) {
        ...
        char c = str.charAt(0);
        return (!Character.isLetter(c) || Character.isUpperCase(c)) ? str : new StringBuffer(str.length())
    ...
    public static String getMD5(String input) {
        try {
            ...
            LogUtils.e(StringUtils.class.getSimpleName(), e);
            return null;
        }
    }
    ...
    // 将字符串按照一定长度分割
    public static String[] stringSpilt(String string, int len){
        int spiltNum;//len->想要分割获得的子字符串的长度
        int cache = len;
        int slength = string.length();

        spiltNum = slength/len;
        ...
        if (slength < len) {
            ...
            int i = 0;
            int start = 0;
            for (i = 0; i < spiltNum; i++) {
                subs[i] = string.substring(start, len);
                start = len;
                len = len + cache;
                if (len > slength) {
                    len = slength;
